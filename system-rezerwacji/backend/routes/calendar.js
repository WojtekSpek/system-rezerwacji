  ...
  router.post("/events", async (req, res) => {
    ...
    try {
      const query = `
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `;
      const [result] = await db.promise().query(query, [title, req.body.start, end1, projectTrainerId, description, projectId, type, participantId, isGroupEvent, groupParticipantIds]);
